...
def sdram_clr_burstdet():
    wb.regs.ddrphy_burstdet_clr.write((1<<N_BYTE_GROUPS)-1)

def sdram_get_burstdet():
    return wb.regs.ddrphy_burstdet_seen.read()
    ...
    class DDRAMReadLeveling:
        ...
        def run(self):
            ...
            for i in range(NDELAYS):
                ...
                burst = sdram_get_burstdet()
                print("burst: 0b{:08b}".format(burst))
                for j in range(16):
                    dq = 0
                    dq |= (p1 >> (16 + j)) & 0b1
                    dq <<= 1
    ...
    def check_pattern(length, debug=False):
        ...
        for i in range(length):
            ...
            if wb.read(wb.mems.main_ram.base + 4*i) != seed_to_data(i):
                error = 1
                if debug:
                    print("{}: 0x{:08x}, 0x{:08x} FAIL".format(i, wb.read(wb.mems.main_ram.base + 4*i), seed_to_data(i)))
            else:
                if debug:
